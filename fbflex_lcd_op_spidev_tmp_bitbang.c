...
spidev1 Wrong !
//	PC3
#define GPIO_LCD_CS (32+32+3)
//	PC2
#define GPIO_LCD_CLK (32+32+2)
//	PC0
#define GPIO_LCD_MOSI (32+32+0)
*/

// undefine=spi, define=bitbang
#define GPIO_MODE_BITBANG 1

#ifdef GPIO_MODE_BITBANG
// spidev1 Correct !
//	spi1_cs0        port:PA13
#define GPIO_LCD_CS (13)
//	spi1_sclk       port:PA14
#define GPIO_LCD_CLK (14)
//	spi1_mosi       port:PA15
#define GPIO_LCD_MOSI (15)
#endif
...
#ifdef GPIO_MODE_BITBANG
	bytes_written = snprintf(buffer, BUFFER_MAX, "%d", GPIO_LCD_CS);
	write(gpioh, buffer, bytes_written);

	bytes_written = snprintf(buffer, BUFFER_MAX, "%d", GPIO_LCD_CLK);
	write(gpioh, buffer, bytes_written);

	bytes_written = snprintf(buffer, BUFFER_MAX, "%d", GPIO_LCD_MOSI);
	write(gpioh, buffer, bytes_written);
#endif
...
#ifdef GPIO_MODE_BITBANG
	snprintf(path, DIRECTION_MAX, "/sys/class/gpio/gpio%d/direction", GPIO_LCD_CS);
	gpioh = open(path, O_WRONLY);
	write(gpioh, "out", 3);
	close(gpioh);

	snprintf(path, DIRECTION_MAX, "/sys/class/gpio/gpio%d/direction", GPIO_LCD_CLK);
	gpioh = open(path, O_WRONLY);
	write(gpioh, "out", 3);
	close(gpioh);

	snprintf(path, DIRECTION_MAX, "/sys/class/gpio/gpio%d/direction", GPIO_LCD_MOSI);
	gpioh = open(path, O_WRONLY);
	write(gpioh, "out", 3);
	close(gpioh);
#endif
...
int spi_transmit(int devsel, uint8_t *data, int len) {
	...
	for (i=0; i<len; ++i) {
		...
		for (j=0; j<8; ++j) {
			if (d & 0x80) {
				gpioWrite(GPIO_LCD_MOSI, 1);
			} else {
				gpioWrite(GPIO_LCD_MOSI, 0);
			}
			d <<= 1;
			gpioWrite(GPIO_LCD_CLK, 0);
			gpioWrite(GPIO_LCD_CLK, 1);
		}
	}

	gpioWrite(GPIO_LCD_CS, 1);
